...
def multi_gaussian_linear(
    ...
    for i in range(ny):
        amp_scale = 1.0 if i == 0 else (1.0 + 0.1 * rng.standard_normal())
        
        y = ls.gaussian(
            x, 
            amplitude=test_gaussian["amplitude"] * amp_scale, 
            center=test_gaussian["center"], 
            sigma=test_gaussian["sigma"]
        )
        y += ls.linear(x, **test_linear)
        y += rng.normal(scale=0.1, size=x.size)
...
def multi_gaussian(
    ...
    for i in range(ny):
        amplitude = +0.60 + 9.50 * rng.random()
        center    = -0.20 + 1.20 * rng.random()
        sigma     = 0.25 + 0.03 * rng.random()
        
        y = ls.gaussian(x, amplitude, center, sigma) + rng.normal(scale=0.1, size=x.size)
        xy[:, i + 1] = y
...
def multi_lorentzian(
    ...
    for i in range(ny):
        amplitude = +1.0 + 3.0 * rng.random()
        center    = -0.5 + 1.0 * rng.random()
        sigma     = 0.3 + 0.1 * rng.random()
        
        y = ls.lorentzian(x, amplitude, center, sigma) + rng.normal(scale=0.05, size=x.size)
        xy[:, i + 1] = y
...
DEMO_DATASETS = {
    ...
    "N-Dataset Gaussian 2": multi_gaussian2,
...
#     """Wrapper function executing dataset generator matching target registry name."""
#     generator = DEMO_DATASETS.get(name, multi_gaussian)
#     return generator(ny=ny, n_points=n_points, seed=seed)
...
def make_demo_data(
    ...
    **kwargs
    ...
    """Generate demo data using the registered dataset generator."""
    generator = DEMO_DATASETS.get(name, multi_gaussian)
